...
_LittleSpecForce = pow(_LittleSpecForce, gamma) * 1.5;
					...
					_Specular3 = pow(_Specular3,gamma);
								...
								//iceValue = saturate(pow((i.color.g), 1 + clamp(abs((snowHeight - 0.5) * 20) * -_TransitionPower * (saturate(i.color.g)), -1, 1)) * 1.25);
								...
								//float3 SnowNormal0 = _Normal0.Sample(my_linear_repeat_sampler, i.uv * (_TerrainScale.xz / _Splat0_ST.xy));
								...
								/*
								SnowNormal0 = lerp(SnowNormal0, SnowNormal0, 1 - _Metallic0);
								float3 SnowNormal1 = UnpackScaleNormal(_Normal1.Sample(sampler_BumpMap, i.uv * (_TerrainScale.xz / _Splat1_ST.xy)), _NormalScale1);
								SnowNormal1 = lerp(SnowNormal1, SnowNormal1, 1 - _Metallic1);
								float3 SnowNormal2 = UnpackScaleNormal(_Normal2.Sample(sampler_BumpMap, i.uv * (_TerrainScale.xz / _Splat2_ST.xy)), _NormalScale2);
								SnowNormal2 = lerp(SnowNormal2, SnowNormal2, 1 - _Metallic2);
								float3 SnowNormal3 = UnpackScaleNormal(_Normal3.Sample(sampler_BumpMap, i.uv * (_TerrainScale.xz / _Splat3_ST.xy)), _NormalScale3);
								...
								*/
								...
								float3 normal = UnpackScaleNormal(tex2D(_BumpMap, (i.uv) * _SnowNormalScale * _SnowScale), _NormalMultiplier * 2).rgb * iceValue - i.normal;
								half3 normalTex = UnpackScaleNormal(tex2D(_NormalTex, i.uv * _IceScale), _NormalScale);
					#endif
								...
								//float3 normalDirectionWithNormal = normalize(i.normalDir) + normalize(i.normalDir) * normalize(abs(tex2D(_BumpMap, i.uv * _SnowNormalScale * _SnowScale))) * _NormalMultiplier;
								...
								for (int j = 0; j < 4; j++) {
									...
									else if (j == 2)
									{
										...
										//underWaterFish += tex2D(_UnderWaterFish, float2((_Time.y * 0.01), -(_Time.y * 0.1)) * 1.4 * _FishSpeed + fishUV).g * 0.75;
									...
									else if (j == 3)
									{
										...
										//underWaterFish += tex2D(_UnderWaterFish, float2((_Time.y * 0.01), -(_Time.y * 0.1)) * 2 * _FishSpeed + fishUV).b * 0.55;
								...
								worldNormal.x = dot(normalDirection.x, lerp(normalTex, UnpackScaleNormal(tex2D(_BumpMap, i.uv * _SnowScale), _NormalMultiplier).rgb * 3 , iceValue));
								worldNormal.y = dot(normalDirection.y, lerp(normalTex, UnpackScaleNormal(tex2D(_BumpMap, i.uv * _SnowScale), _NormalMultiplier).rgb * 3 , iceValue));
								worldNormal.z = dot(normalDirection.z, lerp(normalTex, UnpackScaleNormal(tex2D(_BumpMap, i.uv * _SnowScale), _NormalMultiplier).rgb * 3 , iceValue));
					...
					#else
								float4 specGloss = pow(tex2D(_SpecGlossMap, i.uv * 2 * _SnowScale), _SpecForce);
